...
<template>
  <app-layout view="hhh lPr fff">
    <app-header class="bg-white">
      <Header>
        <bs-btn-menu @click="drawer = true" class="my-auto d-xl-none" />
      </Header>
    </app-header>
    <app-footer>
      <div id="app-footer-content" />
    </app-footer>
    <!-- <app-drawer v-model="drawer" width="55" :breakpoint="'xl'" class="bg-white border-end">
      <template #header>
        <div class="hstack p-1">
          <bs-btn-close @click="drawer = false" class="ms-auto" />
        </div>
      </template>
<IndexDrawer @click="drawer = false" :tab="tab" />
</app-drawer> -->

    <app-drawer v-model="drawer" width="55" breakpoint="xl" v-slot="{ down }" class="border-end">
